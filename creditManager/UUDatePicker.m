...
#define UU_GRAY [UIColor colorWithHexColorString:@"8e979a"];
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int i=[minDateModel.year integerValue]; i<=[maxDateModel.year integerValue]; i++) {
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    yearIndex = [model.year intValue]-[minDateModel.year integerValue];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay)
    {
        NSLog(@"%d",[maxDateModel.year integerValue]-[minDateModel.year integerValue] + 1);
        if (component == 0) return [maxDateModel.year integerValue]-[minDateModel.year integerValue] + 1;
        if (component == 1) return UUPICKER_MONTH;
        if (component == 2){
            return [self DaysfromYear:[yearArray[yearIndex] integerValue] andMonth:[monthArray[monthIndex] integerValue]];
        }
